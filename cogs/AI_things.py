...
class AI_things(commands.Cog):
    ...
    async def askGigachat(self, ctx, *, prompt: str = "Привет!"):
        ...
        async with ctx.typing():
            payload = [{"role": "system", "content": "Не выдавай одни и те же фразы много раз подряд."}, #Если в ответе ты начинаешь повторять одно и то же, перкрати ответ.
    ...
    async def askATKGPT(self, ctx, *, prompt: str = "Привет!"):
        ...
        async with ctx.typing():
            payload = [
                {"role": "system", "content": "Ты ATK GPT. Искуственный интеллект, задача которого - помогать людям.\nЕсли в ответе ты начинаешь повторять одно и то же, перкрати ответ."},
                {"role": "user", "content": prompt}]
            # print(payload)
            response = await AIIO.askLLM(payload, AIIO.LLMs.GIGACHAT, 3, AIIO.gigachat_temptoken)
            print(response)
    ...
    async def kandinsky(self, ctx, *, prompt:str):
        ...
        async with ctx.typing():
            embed = discord.Embed(title="Запуск генерации...",description=f"**Запрос:** {prompt}\n\n***Генерация может занять немного времени...***",colour=Data.getEmbedColor(Data.EmbedColor.Neutral))
            ...
            if gen['censored']:
                embed = discord.Embed(title="Ошибка генерации!",description="NSFW изображения не разрешены этой моделью!",colour=Data.getEmbedColor(Data.EmbedColor.Error))
                await ctx.reply(embed=embed)
            else:
                if gen["image"]=="Error":
                ...
                embed.colour=Data.getEmbedColor(Data.EmbedColor.Success)
                embed.add_field(name="Время генерации",value=f"{round(time.time()-time_start, 2)} сек.",inline=False)
